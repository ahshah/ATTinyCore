...
void USI_TWI_Master_Initialise(void) {
  ...
  USICR = (0 << USISIE) | (0 << USIOIE) |                 // Disable Interrupts.
          ...
          (1 << USICS1) | (0 << USICS0) | (1 << USICLK) | // Software clock strobe as source, via USITC
          (0 << USITC);                                   // No reason to strobe a zero.
...
unsigned char USI_TWI_Start_Transceiver_With_Data_Stop(unsigned char *msg, unsigned char msgSize, unsigned char stop) {
  ...
  if (USISR & (1 << USISIF)) {
    USISR = 1 << USISIF;    // reset the start condition, generated by noise
    return (FALSE);         // come back next time
  }
/*
  if (USISR & (1 << USIOIF)) {
    USISR = 1 << USIOIF;    // reset the start condition, generated by noise
  }
  */
